...
def run_once(base_url: str, app_name: str, user_id: str, session_id: str, query: str, doc_path: str, api_key: str = None) -> None:
    run_endpoint = f"{base_url}/run"
    headers = {"Content-Type": "application/json", "Accept": "application/json"}
    if api_key:
        headers["x-api-key"] = api_key
    else:
        print("⚠️  No API key provided; request will likely be rejected.")
...
if __name__ == "__main__":
    ...
    p.add_argument("--api_key", default="dev-secret", help="API key for authentication (x-api-key header)")
    args = p.parse_args()
